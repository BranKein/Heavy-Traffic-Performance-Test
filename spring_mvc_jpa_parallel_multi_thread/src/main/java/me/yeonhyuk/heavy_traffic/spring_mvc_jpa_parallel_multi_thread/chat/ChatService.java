...
import me.yeonhyuk.heavy_traffic.spring_mvc_jpa_parallel_multi_thread.common.ApiException;
import me.yeonhyuk.heavy_traffic.spring_mvc_jpa_parallel_multi_thread.common.ResultCodeEnum;
...
import me.yeonhyuk.heavy_traffic.spring_mvc_jpa_parallel_multi_thread.push.PushClient;
import me.yeonhyuk.heavy_traffic.spring_mvc_jpa_parallel_multi_thread.repository.ChatRepository;
import me.yeonhyuk.heavy_traffic.spring_mvc_jpa_parallel_multi_thread.repository.ChatRoomRepository;
import me.yeonhyuk.heavy_traffic.spring_mvc_jpa_parallel_multi_thread.repository.DeviceRepository;
import me.yeonhyuk.heavy_traffic.spring_mvc_jpa_parallel_multi_thread.repository.UserChatRoomRepository;
 ...
 *
 * <p>Parallel(platform-thread) 버전: 각 device 에 대한 푸시(device 당 0.5초 소요)를 순차
 * 호출하지 않고 고정 크기 플랫폼 스레드 풀 위에서 동시에 fan-out 한다. device 수가 풀 크기
 * 이하면 지연이 사실상 단일 푸시(≈0.5초)에 수렴하지만, 초과분은 큐에서 대기한다.
 * 가상 스레드 버전(spring_mvc_jpa_parallel_virtual_thread)·동기 순차 버전(spring_mvc_jpa)과의 비교군이다.
 *
...
public class ChatService {
    ...
    public UUID sendChat(UUID userPk, UUID chatRoomPk, String chatData) {
        ...
        //    device 마다 CompletableFuture 를 스레드 풀에 fan-out 하고 전부 완료될 때까지 대기한다.
